...
public class EllipticCurve {
	...
	private Point BasePoint;// the base Point or generator
	...
	public EllipticCurve (BigInteger a , BigInteger p,Point basePoint, BigInteger n , BigInteger h ) { // construcure to initilize the domain paramters
...
// return 1 , if the x coordiates are equal AND the result of adding the y coordiates are equal to the prime number P
	...
	Point returnValue = new Point (BigInteger.ZERO , BigInteger.ZERO,BigInteger.ZERO);
	 ...
	 private static BigInteger selectPrivatekey (EllipticCurve EC) {
			
			BigInteger maxLimit = EC.getN();
			BigInteger minLimit = new BigInteger("1");
			BigInteger private_Key=getRandomNumber(minLimit ,maxLimit);
			return private_Key;
			
		}
	  
	  private  Point computePublic_key ( BigInteger Private_Key ) throws Exception {
		  Point Public_key =scalarMultplication(Private_Key, this.BasePoint);
			return Public_key;
			
		}
	  private  BigInteger selectPrivatekey () {
			
			BigInteger maxLimit = this.getN();
			BigInteger minLimit = new BigInteger("1");
			BigInteger private_Key=getRandomNumber(minLimit ,maxLimit);
			return private_Key;
	  ...
	  private static BigInteger getRandomNumber(BigInteger minLimit , BigInteger maxLimit) {
